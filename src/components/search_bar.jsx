...
const SearchBar = () => {
  ...
  const handleChange = (event) => {
    ...
    if (query.trim() === "") {
      ...
      setSuggestions(
        products.filter(
          (p) => p.name.toLowerCase().includes(query.toLowerCase()), // Búsqueda parcial
        ),
      );
    }
  };
          ...
          {suggestions.length > 0 && (
            ...
            >
              {suggestions.map((suggestion, index) => {
                const { productoTipoPars, categoriaPars } = parsear(
                  suggestion.typeProduct,
                  suggestion.category,
                );
                ...
                return (
                  <li
                    ...
                    onClick={() =>
                      nv(
                        `/shop/${productoTipoPars}/${categoriaPars}/${suggestion.name}`,
                      )
                    } //}
